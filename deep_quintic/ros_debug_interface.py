...
class ROSDebugInterface:
    def __init__(self, env):
        ...
        # publisher
        self.left_foot_pressure_publisher = self.env.node.create_publisher(FootPressure, "debug/foot_pressure_raw/left", 1)
        self.right_foot_pressure_publisher = self.env.node.create_publisher(FootPressure, "debug/foot_pressure_raw/right", 1)
        self.left_foot_pressure_publisher_filtered = self.env.node.create_publisher(FootPressure, "debug/foot_pressure_filtered/left", 1)
        self.right_foot_pressure_publisher_filtered = self.env.node.create_publisher(FootPressure, "debug/foot_pressure_filtered/right", 1)
        self.joint_publisher = self.env.node.create_publisher(JointState, "debug/joint_states", 1)
        self.imu_publisher = self.env.node.create_publisher(Imu, "debug/imu/data", 1)
        self.imu_truth_publisher = self.env.node.create_publisher(Imu, "debug/imu/truth", 1)
        self.imu_rpy_publisher = self.env.node.create_publisher(Float32MultiArray, "debug/imu_rpy", 1)
        self.clock_publisher = self.env.node.create_publisher(Clock, "debug/clock", 1)
        self.real_time_factor_publisher = self.env.node.create_publisher(Float32, "debug/real_time_factor", 1)
        ...
        self.refbot_pose_publisher = self.env.node.create_publisher(PoseStamped, "debug/ref_pose", 1)
        self.lin_vel_publisher = self.env.node.create_publisher(Float32MultiArray, "debug/lin_vel", 1)
        self.ang_vel_publisher = self.env.node.create_publisher(Float32MultiArray, "debug/ang_vel", 1)
