...
class ROSInterface:
    def __init__(self, simulation, init_node=True):
        if init_node:
    ...
    def compute_real_time_factor(self):
        time_now = time.time()
        self.real_time_msg.data = self.simulation.env_timestep / (time_now - self.last_time)
    ...
    def publish_imu(self):
        if self.imu_publisher.get_num_connections() > 0:
            self.imu_publisher.publish(self.simulation.get_imu_msg())
    ...
    def publish_true_odom(self):
        if self.true_odom_publisher.get_num_connections() > 0:
            self.odom_msg.pose.pose.position = Point(*self.simulation.pos_in_world)
            self.odom_msg.pose.pose.orientation = Quaternion(*wxyz2xyzw(self.simulation.quat_in_world))
